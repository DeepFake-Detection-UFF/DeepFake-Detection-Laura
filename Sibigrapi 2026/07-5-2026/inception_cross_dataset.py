...
if not os.path.exists(RESULTS_FILE):
    with open(RESULTS_FILE, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["experiment", "accuracy", "f1", "auc"])
...
def train(model, loader):
    ...
    # ⚠️ desbalanceamento DFDC → mais peso pra REAL
...
# ---------------------------
print("\n" + "="*60)
print("CROSS DATASET: FF++ → DFDC")
print("="*60)

train_dataset = FFPPBinaryDataset(FFPP_DIR, "train", transform)
train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)

test_dataset = DFDCDataset(DFDC_DIR, "test", transform)
...
with open(RESULTS_FILE, "a", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["FF++_to_DFDC", acc, f1, auc])